...
def poedit(request):
    if rosetta_settings.PATH_TO_POEDIT:
        call(
            [rosetta_settings.PATH_TO_POEDIT, request.GET.get("file", None)],
            cwd="/tmp",
        )
    return HttpResponseRedirect("/rosetta/")


def find_all_locale(path):
    result = []
    for root, dirs, files in os.walk(path):
        if "locale" in dirs:
            result.append(os.path.join(root, "locale"))
    return result
