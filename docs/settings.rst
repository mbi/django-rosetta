...
* ``ROSETTA_HIDE_TRANSLATORS_ON_CONTEXT_HINTS``: Allow to hide in user interface the word ``Translators`` at the beginning of the extracted comment generated by Django using ``--add-commments=Translators`` in ``xgettext`` call. Defaults to ``False``.
